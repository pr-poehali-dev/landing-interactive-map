    ...
    >
      ...
      {open && (
        <div className="lg:hidden bg-white border-t px-5 pb-4 pt-2 space-y-1 shadow-lg">
          {NAV.map((n) => (
            <Link
              ...
              className={`block px-4 py-2.5 text-sm font-medium rounded-lg ${
                pathname === n.to ? "text-v-600 bg-v-50" : "text-gray-400 hover:bg-gray-50"
              }`}
            >
              {n.label}
            </Link>
          ...
          <Link
            to="/dashboard"
            className="block px-4 py-2.5 text-sm font-medium rounded-lg text-gray-400 hover:bg-gray-50"
          >
            Личный кабинет
          </Link>
          <Link
