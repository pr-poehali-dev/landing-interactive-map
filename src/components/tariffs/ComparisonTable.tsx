...
export default function ComparisonTable() {
  return (
    <section className="py-20 px-5 bg-[#F1F1F8]">
          ...
          <table className="w-full min-w-[800px] text-sm">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left font-semibold text-foreground py-4 px-6 bg-[#F1F1F8] sticky left-0 z-10">
                  Функция
                </th>
                {COMPARE_PLANS.map((plan) => (
                  <th key={plan} className="text-center font-semibold text-foreground py-4 px-4 bg-[#F1F1F8]">
            ...
            <tbody>
              {COMPARE_ROWS.map((row, ri) => (
                <tr
                  key={row.label}
                  className={`border-b border-gray-100 last:border-0 ${ri % 2 === 0 ? "" : "bg-gray-50/50"}`}
                >
